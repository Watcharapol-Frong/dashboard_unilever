...
export async function GET(request: NextRequest) {
  ...
  const agentMap = new Map<string, { agent_name: string; agent_company: string | null; total_calls: number; contacted: number; interested: number; ordered: number }>()
  ...
  const { data: leads } = await supabase.from('leads').select('customer_id').gte('assigned_date', from).lte('assigned_date', to)
  const leadCount = (leads ?? []).length || allCalls.length
  ...
  const firstOrders = (onlineOrders ?? []).length
  ...
  const sankey = {
    nodes: [
      ...
      if (n.id === 'Lead List') return leadCount > 0
    ...
    links: [
      leadCount > 0 && allCalls.length > 0 ? { source: 'Lead List', target: 'Called', value: Math.min(leadCount, allCalls.length) } : null,
  ...
  return NextResponse.json({ summary, by_agent, by_date, sankey })
}
