...
export async function GET(request: NextRequest) {
  ...
  const interested   = calls.filter(c => ['Interested','Ordered'].includes(c.call_status)).length
  const ordered      = calls.filter(c => c.call_status === 'Ordered').length
  const allCustIds   = new Set([...(onlineCustRes.data ?? []).map(r => r.customer_id), ...(offlineCustRes.data ?? []).map(r => r.customer_id)])
  const total_customers     = allCustIds.size
  ...
  if (prev_from && prev_to) {
    ...
    prev_contacted     = (pCalls.data ?? []).filter((c: { call_status: string }) => ['Contacted','Interested','Not Interested','Ordered'].includes(c.call_status)).length
    prev_interested    = (pCalls.data ?? []).filter((c: { call_status: string }) => ['Interested','Ordered'].includes(c.call_status)).length
    prev_ordered       = (pCalls.data ?? []).filter((c: { call_status: string }) => c.call_status === 'Ordered').length
    prev_new_customers = typeof pNewCust.data === 'number' ? pNewCust.data : 0
  }

  return NextResponse.json({
    new_customers, new_customers_per_day: new_customers / days,
    returning_customers, retention_rate: total_customers > 0 ? returning_customers / total_customers : 0, total_customers,
    total_sales, total_sales_online, total_sales_offline, order_count, aov,
    sales_target, target_pct: sales_target > 0 ? total_sales / sales_target : 0,
    total_calls, calls_per_day: total_calls / days,
    connection_rate: total_calls > 0 ? contacted / total_calls : 0, contacted,
    conversion_count: ordered, conversion_rate: contacted > 0 ? ordered / contacted : 0,
    engaged: interested, engaged_rate: contacted > 0 ? interested / contacted : 0,
    prev_new_customers, prev_total_sales, prev_total_calls,
    prev_conversion_rate: prev_contacted > 0 ? prev_ordered / prev_contacted : 0,
    prev_engaged_rate:    prev_contacted > 0 ? prev_interested / prev_contacted : 0,
  })
}
