...
export const leadsColumns: ColumnDef<Lead>[] = [
  ...
  {
    accessorKey: 'primary_cmg',
    header: ({ column }) => <DataTableColumnHeader column={column} title="CMG" />,
    cell: ({ row }) => (
      <span className="text-xs text-muted-foreground">
        {row.original.primary_cmg ?? <span className="text-muted-foreground/40">—</span>}
      </span>
    ),
  },
  {
  ...
  },
  {
    ...
    header: ({ column }) => <DataTableColumnHeader column={column} title="HOC Sales" className="justify-end" />,
    cell: ({ row }) => (
      <div className="text-right tabular-nums text-sm">
