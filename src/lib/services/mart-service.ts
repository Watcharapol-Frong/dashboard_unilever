...
const buildInsertSQL = (withMmidFilter: boolean) => `
  ...
  INSERT INTO mart_telesales_orders (
    ...
    product_name_th, product_name_en, brands, class_name, is_hoc_unilever, month, refreshed_at
  )
  SELECT
    mmid, order_number, order_date, channel, prod_num, sales_qty, sales_in_vat,
    dynamic_cmg, first_connected_date, agent, call_status, lead_customers,
    ...
    TRUE AS is_hoc_unilever, month, NOW() AS refreshed_at
  FROM ranked
...
export async function buildMartCostIncentive(): Promise<number> {
  ...
  await query(`
    ...
    tier_calls AS (
      ...
      FROM telesales_calls WHERE first_connected_date IS NOT NULL GROUP BY 1, 2
    ),
    base AS (
      SELECT cm.month, cm.dynamic_cmg, tc.lead_customers, tc.total_calls, tc.reached,
        asc2.actual_sales,
      ...
      FROM (SELECT month, dynamic_cmg FROM all_sales_by_cmg) cm
