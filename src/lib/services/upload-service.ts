...
async function processUploadFromText(type: UploadFileType, text: string, filename: string): Promise<UploadResult> {
  ...
  const batch = await queryOne<{ id: string }>(
    `INSERT INTO upload_batches (table_name, filename, storage_path, status) VALUES ($1, $2, $3, 'failed') RETURNING id`,
  ...
  if (!dbError && upsertRows.length > 0) {
    try {
      if (type === 'online_sales' || type === 'offline_sales') {
        ...
        await query(
          `INSERT INTO table_summaries (table_name, total_rows, total_sales) VALUES ($1, $2, $3)
        ...
        await query(
          `INSERT INTO table_summaries (table_name, total_rows) VALUES ($1, $2)
           ON CONFLICT (table_name) DO UPDATE SET
             total_rows = table_summaries.total_rows + EXCLUDED.total_rows,
             last_updated = NOW()`,
